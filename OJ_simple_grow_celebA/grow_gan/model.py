...
def G(z, batch_size=64, reuse = False, bn = True, layers = 12, activation = 'lrelu', output_dim = 128,
    feature_map_shrink = 'n', spatial_map_growth = 'n', use_wscale = True, use_pixnorm = True, beta = 1, useBeta = 'n'):
    with tf.variable_scope("generator") as scope:
        ...
        for i in range(layers):
            if i == 0:
                ...
                h = act(conv4x4(z, 256*4*4, batch_size, name = 'g_h'+str(i+1), useBeta = useBeta, beta = beta, use_wscale = use_wscale), activation)
...
def D(image, batch_size=64, reuse = False, bn = True, layers = 12, activation = 'lrelu', input_dim = 128,
    feature_map_growth = 'n', spatial_map_shrink = 'n', z_dim = 8, minibatch_std = True, use_wscale = True, beta = 1, useBeta = 'n'):
        ...
        for i in range(layers):
            if i == 0:
                ...
                if feature_map_growth == 'n' or feature_map_growth == 'pro' or feature_map_growth == 'cpro':
                    if i in featureUpsampleList and int(h.get_shape()[-1]) < 512: #z_dim:
                    ...
                    if int(h.get_shape()[-1]) < 512: #z_dim:
                        h = act(conv2d(h, int(h.get_shape()[-1])*2, 3, 3, 1, 1, name='d_h'+str(i+1), padding = 'SAME', use_wscale = use_wscale), activation)
                        print('d_h'+str(i+1)+':', h.get_shape())
                    else:
                        if i == layers -2 and minibatch_std:

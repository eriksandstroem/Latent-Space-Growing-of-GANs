...
def pixel_norm(x, epsilon=0.0, useGamma = 'n', gamma = 1):#1e-8 for epsilon before
    with tf.variable_scope('PixelNorm'):
        if useGamma == 'y':
            return x * tf.rsqrt(1/gamma*tf.reduce_sum(tf.square(x), axis=3, keepdims=True) + epsilon)
        else:

...
def G(z, batch_size=64, reuse = False, bn = True, layers = 12, activation = 'lrelu', output_dim = 128,
    feature_map_shrink = 'n', spatial_map_growth = 'n', stage = 'f', alpha = 1):
    with tf.variable_scope("generator") as scope:
        if reuse:
            scope.reuse_variables()
...
def D(image, batch_size=64, reuse = False, bn = True, layers = 12, activation = 'lrelu', input_dim = 128,
    feature_map_growth = 'n', spatial_map_shrink = 'n', stage = 'f', alpha = 1, z_dim = 8): # stage = ['final', 'intermediate'] (['f', 'i'])
    with tf.variable_scope("discriminator") as scope:
        if reuse:
            scope.reuse_variables()
        ...
        for i in range(layers):
            if i == 0:
                ...
                h = conv2dVALID(h, int(h.get_shape()[-1]), 4, 4, 1, 1, name = 'd_h'+str(layers))
                print('d_h'+str(i+1)+':', h.get_shape())
            else:
                if spatial_map_shrink == 'n' and i in downsampleList and int(h.get_shape()[1]) > 4: #and i != 1 and (i+1) % 2 == 0
                    ...
                    h = downscale2d(h, factor=2)
                if feature_map_growth == 'n':
                    if i in downsampleList and stage == 'f' and int(h.get_shape()[-1]) < z_dim: # i % 2 == 0
                        h = conv2d(h, int(h.get_shape()[-1])*2, 3, 3, 1, 1, name='d_h'+str(i+1), stddev = 
                        np.sqrt(2/(int(h.get_shape()[-1])*int(h.get_shape()[1])*int(h.get_shape()[2]))))
                        print('d_h'+str(i+1)+':', h.get_shape())
        ...
        out = tf.layers.dense(h, 1, activation=None, name = 'd_out',
